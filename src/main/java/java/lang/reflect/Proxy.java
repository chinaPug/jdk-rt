...
public class Proxy implements java.io.Serializable {
    ...
    /**
     * 返回一个代理类的实例，该实例为指定的接口实现方法调用分派到指定的调用处理程序。
     *
     * <p>{@code Proxy.newProxyInstance} 在以下情况下会抛出
     * {@code IllegalArgumentException}，原因与 {@code Proxy.getProxyClass} 抛出异常的原因相同。
     *
     * @param   loader 用于定义代理类的类加载器
     * @param   interfaces 代理类需要实现的接口列表
     * @param   h 方法调用分派的目标调用处理程序
     * @return  一个代理实例，该实例具有指定的调用处理程序，并由指定的类加载器定义，
     *          同时实现了指定的接口
     * @throws  IllegalArgumentException 如果传递给 {@code getProxyClass} 的参数违反了任何限制
     * @throws  SecurityException 如果存在安全管理器 <em>s</em>，并且满足以下任一条件：
     *          <ul>
     *          <li> 给定的 {@code loader} 为 {@code null}，而调用者的类加载器不为 {@code null}，
     *               并且调用 {@link SecurityManager#checkPermission s.checkPermission}
     *               使用权限 {@code RuntimePermission("getClassLoader")} 被拒绝访问；</li>
     *          <li> 对于每个代理接口 {@code intf}，调用者的类加载器不是或不是 {@code intf} 类加载器的祖先，
     *               并且调用 {@link SecurityManager#checkPackageAccess s.checkPackageAccess()} 拒绝访问 {@code intf}；</li>
     *          <li> 给定的代理接口是非公共的，并且调用类不在与非公共接口相同的运行时包中，
     *               调用 {@link SecurityManager#checkPermission s.checkPermission} 使用权限
     *               {@code ReflectPermission("newProxyInPackage.{package name}")} 被拒绝访问。</li>
     *          </ul>
     * @throws  NullPointerException 如果 {@code interfaces} 数组参数或其任何元素为 {@code null}，
     *          或者如果调用处理程序 {@code h} 为 {@code null}
     */
    @CallerSensitive
